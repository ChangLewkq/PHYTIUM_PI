...
/**
  * 函    数：获取编码器累计值
  * 参    数：n 指定左电机还是右电机，范围：1（左电机），2（右电机）
  * 返 回 值：编码器累计值（32位，不会溢出）
  */
int32_t Encoder_GetTotal(uint8_t n)
{
    static int32_t TotalLeft = 0;       // 左编码器累计值
    static int32_t TotalRight = 0;      // 右编码器累计值
    int16_t Delta;
    
    if (n == 1)
    {
        Delta = TIM_GetCounter(TIM3);
        TIM_SetCounter(TIM3, 0);
        TotalLeft += Delta;
        return TotalLeft;
    }
